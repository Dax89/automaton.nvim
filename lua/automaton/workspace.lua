...
return function(config, rootpath)
    ...
    function Workspace:ws_root()
        return vim.fs.joinpath(self.rootpath, config.impl.workspace)
    end

    function Workspace:ws_open(filename)
        vim.api.nvim_command(":e " .. vim.fs.joinpath(self:ws_root(), filename))
    end
    ...
    function Workspace:get_name() return vim.fs.basename(self.rootpath) end
    ...
    function Workspace:_load_json(filename, fallback, schema)
        ...
        local filepath = vim.fs.joinpath(self:ws_root(), filename)
    ...
    function Workspace:update_state(state)
        local statepath = vim.fs.joinpath(self:ws_root(), config.impl.statefile)
    ...
    function Workspace:get_launch()
        ...
        if type(wslaunch) == "table" then
            return Variable.resolve(wslaunch.configurations or {},
    ...
    function Workspace:tasks_default()
        ...
        if task then
            ...
            vim.notify("Default task not found")
        end
    end
